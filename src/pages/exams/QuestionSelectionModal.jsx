...
const QuestionSelectionModal = ({
    ...
    const [filter, setFilter] = useState({ level: levelName || '', type: sectionType || '' });
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1);
    const [selectedQuestions, setSelectedQuestions] = useState(initialSelectedQuestions);
    const [selectedQuestionForDetail, setSelectedQuestionForDetail] = useState(null);
    ...
    const columns = [
        {
            ...
            render: (value, row) => (
                <input
                    ...
                    disabled={selectedQuestions.length >= quantityLimit && !selectedQuestions.some(sq => sq.id === row.id)}
        ...
        setSelectedQuestions(prevSelected => {
            ...
            if (isAlreadySelected) {
                ...
                if (prevSelected.length < quantityLimit) {
                    ...
                    toast.warn(`Bạn chỉ có thể chọn tối đa ${quantityLimit} câu hỏi cho phần này.`);
                    return prevSelected;
                }
            }
        });
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 w-11/12 max-w-4xl max-h-[90vh] overflow-y-auto relative">
                ...
                <h2 className="text-2xl font-bold mb-4 text-indigo-700">Chọn câu hỏi cho phần "{levelName} - {sectionType}"</h2>
                <p className="text-gray-600 mb-4">Bạn cần chọn  {quantityLimit} câu hỏi.</p>
                <p className="text-gray-600 mb-4">Đã chọn: {selectedQuestions.length} / {quantityLimit} câu hỏi.</p>
