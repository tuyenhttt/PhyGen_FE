...
const ExamPaperPreview = ({ examData, sectionsCreated, matrixSectionDetail, selectedQuestionsBySection, onBack, onQuestionsSelected }) => {
    ...
    const sortedSections = [...sectionsCreated].sort((a, b) => a?.data?.displayOrder - b?.data?.displayOrder);
    ...
    const orderedGroupedSections = Object.values(groupedSections).sort((a, b) => a.displayOrder - b.displayOrder);
    
    const handleOpenQuestionModal = (sectionId, sectionType, levelName, quantityLimit) => {
        setCurrentSectionToSelect({ id: sectionId, type: handleSectionType(sectionType), level: levelName });
    ...
    return (
        <div className='bg-white p-6 rounded-lg shadow-md space-y-6'>
            ...
            <h3 className='text-xl font-bold text-indigo-700 text-center mb-4'>Cấu trúc đề thi</h3>
                ...
                <div key={group.title} className="border border-gray-200 p-4 rounded-md mt-4">
                    <h4 className='text-lg font-semibold text-gray-800 mb-3'>
                        {group.title}
                    </h4>
                    {group.details.map(s => {
                        const correspondingMatrixDetail = matrixSectionDetail.find(
                            (detail) => detail?.typeName === s?.data?.sectionType && detail?.levelName === s?.levelName
                        );

                        const quantity = correspondingMatrixDetail ? correspondingMatrixDetail?.quantity : 'N/A';
                        const currentSelectedCount = selectedQuestionsBySection[s?.data?.id]?.length || 0; 

                        return (
                            <div key={s?.data?.id} className="ml-4 border-l-2 border-indigo-300 pl-4 py-2 my-2 bg-gray-50 rounded-md">
                                ...
                                <div className="flex items-center justify-between p-2 bg-white border border-dashed border-gray-200 rounded-md">
                                    ...
                                    <PrimaryButton
                                        ...
                                        onClick={() => handleOpenQuestionModal(s?.data?.id, s?.data?.sectionType, s?.levelName, quantity)}
                                    >
                                        Chọn câu hỏi
                                    </PrimaryButton>
                                </div>
                                <ul className="mt-2">
                                    {selectedQuestionsBySection[s?.data?.id]?.map((q, index, array) => (
                                        <li 
                                            key={q?.id} 
                                            className={`text-gray-700 text-sm flex items-start ${
                                                index < array.length - 1 ? 'border-b border-gray-200 pb-2 mb-2' : ''
                                            }`}
                                        >
                                            <span className="font-semibold mr-2 flex-shrink-0">{q?.no}.</span>
                                            <span style={{ whiteSpace: 'normal' }}>{q?.content}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
            ...
            {isQuestionModalOpen && currentSectionToSelect && (
                <QuestionSelectionModal
                    ...
                    initialSelectedQuestions={selectedQuestionsBySection[currentSectionToSelect.id] || []}
