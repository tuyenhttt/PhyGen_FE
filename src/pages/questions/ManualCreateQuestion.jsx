...
const ManualCreateQuestion = forwardRef(({ }, ref) => {
    ...
    const [answerOptions, setAnswerOptions] = useState([{ id: 1, text: '', isCorrect: false }]);
    ...
    useImperativeHandle(ref, () => ({
        ...
        validate: () => {
            if (!content || !selectedTopicId || !questionType || !level) {
                alert('Vui lòng điền đầy đủ thông tin câu hỏi (Nội dung, Chủ đề, Loại, Cấp độ).');
                return false;
            }
            if ((questionType === '0' || questionType === '1') && answerOptions.length === 0) {
            ...
            if ((questionType === '0' || questionType === '1') && correctAnswers.length === 0) {
        ...
        const fetchTopic = async () => {
            ...
            try {
                ...
                if (response.data && Array.isArray(response.data.data) && response.data.data.length > 0) {
                    const validTopics = response.data.data.filter(t => Object.keys(t).length > 0 && t.id && t.name);
    ...
    const handleAddAnswerOption = () => {
        if (answerOptions.length < 6) {
            setAnswerOptions([...answerOptions, { id: answerOptions.length + 1, text: '', isCorrect: false }]);
        }
    };
    ...
    const handleCorrectAnswerChange = (id) => {
        setAnswerOptions(answerOptions.map(option =>
            ({ ...option, isCorrect: option.id === id ? !option.isCorrect : false })
        ));
    };
    ...
    return (
        <div className='space-y-4'>
            ...
            <div className='relative' ref={topicRef}>
                ...
                {showTopicSuggestions && !loadingTopic && filteredTopics.length === 0 && topicSearchInput.length > 0 && (
                    ...
                    {answerOptions.map((option) => (
                        <div key={option.id} className='flex items-center space-x-2 mb-2'>
                            ...
                            <input
                                ...
                                value={option.text}
                                onChange={(e) => handleAnswerTextChange(option.id, e.target.value)}
                            />
                        </div>
                    ))}
                    <PrimaryButton
            ...
            {imagePreviewUrl && (
                <div className='mt-4'>
                    <img src={imagePreviewUrl} alt='Xem trước hình ảnh' className='max-w-full h-auto rounded' />
                </div>
            )}
        </div>
    );
