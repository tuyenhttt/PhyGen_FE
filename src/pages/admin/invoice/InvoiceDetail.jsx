...
const InvoiceDetail = () => {
    ...
    const data = [
        {
            ...
            items: [
                { name: "Gói 400 Coin", qty: 1, price: 40000, total: 400 },
            ],
        ...
        },
        {
            ...
            paymentMethod: 'Visa',
            items: [
                { name: "Gói 100 Coin", qty: 1, price: 10000, total: 100 },
            ],
            status: 'Hủy',
        },
        {
            ...
            total: '134 VNĐ',
            items: [
                { name: "Gói 200 Coin", qty: 1, price: 20000, total: 200 },
            ],
        ...
        },
        {
            ...
            paymentMethod: 'Paypal',
            items: [
                { name: "Gói 500 Coin", qty: 1, price: 500000, total: 500 },
            ],
            status: 'Thất bại',
        },
    ];
    ...
    const getStatusColor = (data) => {
        switch (data) {
            case "Đã thanh toán": return "bg-green-100 text-green-600";
            case "Chờ thanh toán": return "bg-yellow-100 text-yellow-600";
            case "Hủy":
            case "Thất bại": return "bg-red-100 text-red-600";
    ...
    return (
        <div className="max-w-5xl mx-auto bg-white p-8 rounded-2xl shadow-md space-y-8">
            ...
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-gray-600 mt-2">
                        1729 Bangor St., Houston, ME, 04730, United States<br />
                        Phone: +1 (342) 532-9109
                    </p>
                </div>
                <div className="text-right space-y-1">
                    <p className="text-sm text-gray-600">Invoice: <strong>{invoice.idInvoice}</strong></p>
                    <p className="text-sm text-gray-600">Ngày thanh toán: {invoice.paymentDate}</p>
                    <p className="text-sm text-gray-600">Tổng cộng: <strong>{invoice.total}</strong></p>
                    <span className={`inline-block px-3 py-1 text-xs font-bold rounded-full ${getStatusColor(invoice.status)}`}>
                        {invoice.status}
                    </span>
                </div>
            </div>
                ...
                <tbody>
                    {invoice.items.map((item, index) => (
                        <tr key={index} className="border-t">
                            ...
                            <td className="py-3 px-4 text-right">{item.price.toLocaleString()} VNĐ</td>
                            <td className="py-3 px-4 text-right">{item.total.toLocaleString()} VNĐ</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            <div className="bg-red-50 border border-red-200 p-4 rounded-md text-sm text-red-800">
                Tất cả hóa đơn cần được thanh toán trong vòng 7 ngày kể từ ngày phát hành. Nếu không, khoản phí dịch vụ đã cung cấp sẽ được tính như xác nhận công việc.
            </div>
