...
const [loading, setLoading] = useState(false);
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      if (checkoutUrl) {
        ...
        toast.error('Không nhận được URL thanh toán từ PayOS. Vui lòng thử lại.');
      }
    } catch (err) {
      console.error('Lỗi khi tạo thanh toán:', err);
      setError(err.response?.data?.message || err.message || 'Đã xảy ra lỗi khi tạo thanh toán. Vui lòng thử lại.');
      toast.error(err.response?.data?.message || err.message || 'Đã xảy ra lỗi khi tạo thanh toán.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className='max-w-3xl mx-auto my-12 p-8 border border-gray-200 rounded-xl shadow-lg bg-white text-gray-800 font-sans mt-20'>
      <h2 className='text-3xl font-bold text-center text-gray-900 mb-4'>Nạp Coin</h2>

      <div className='bg-gray-50 p-6 rounded-lg mb-4 border-l-4 border-blue-500'>
        <h3 className='text-xl font-semibold mb-4 text-gray-700'>Tạo hóa đơn</h3>
        <p><strong>Họ và tên:</strong> {user.fullName}</p>
        <p><strong>Email:</strong> {user.email}</p>
      </div>

      <div className='bg-gray-50 p-6 rounded-lg mb-4 border-l-4 border-blue-500'>
        <h3 className='text-xl font-semibold mb-4 text-gray-700'>Số lượng Coin</h3>
      ...
      <div className='bg-gray-50 p-6 rounded-lg mb-4 border-l-4 border-blue-500'>
        ...
        <div className='flex justify-between items-center'>
          ...
          <span className='text-2xl font-bold text-blue-600'>{totalAmount.toLocaleString('vi-VN')} VNĐ</span>
        </div>
      </div>
      ...
      >
        {loading ? 'Đang chuyển hướng đến PayOS...' : 'Thanh toán ngay với PayOS'}
      </PrimaryButton>
